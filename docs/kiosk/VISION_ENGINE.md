...
| `FaceDetector` | `backend/app/vision/face_detector.py` | Runs HOG face detection and landmark extraction for every frame accepted by backpressure. The 30 fps figure is a capture/detection target; actual throughput depends on kiosk hardware and is measured in developer diagnostics. |
| `FaceTracker` | `backend/app/vision/face_tracker.py` | Assigns temporary Track IDs using one-to-one IoU matching, resets evidence after movement, and expires lost tracks. |
...
| `RecognitionService` | `backend/app/vision/recognition_service.py` | Enforces a 500 ms recognition cadence and delegates matching to the existing `FaceService`. |
...
- Detection runs on every accepted frame. Recognition runs at most every 500 ms for a stable, quality-approved track.
...
No live frame is stored by this pipeline. The existing matcher temporarily needs a path, so an eligible recognition frame is written only inside an automatically deleted private temporary directory.
