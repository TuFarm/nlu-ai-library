...
export function TrackingDiagnostics({ developer = false }: { developer?: boolean }) {
  ...
  useEffect(() => kioskEvents.subscribe(({ event, payload }) => {
    ...
    if (event === "recognition_progress") setConfidence(values => ({ ...values, [Number(payload.track_id)]: Number(payload.confidence ?? 0) }));
    if (event === "camera_stopped") { setFaces([]); setConfidence({}); }
  }), []);
  return <svg className="tracking-diagnostics" preserveAspectRatio="xMidYMid meet" viewBox={`0 0 ${size[0]} ${size[1]}`} aria-label="Face tracking overlay">
    {faces.map(face => {
      const [top, right, bottom, left] = face.box;
      const score = confidence[face.track_id] ?? 0;
      ...
      return <g className={`tracking-face ${face.quality_ok ? "good" : "waiting"}`} key={face.track_id} fill="none">
        ...
        <circle className="confidence-progress" cx={circleX} cy={circleY} r="19" pathLength="100" strokeDasharray={`${Math.round(score*100)} 100`}/>
        {developer && <>
          <text x={size[0]-right} y={Math.max(22, top-10)}>Track #{face.track_id}</text>
          <text x={circleX} y={circleY+5} textAnchor="middle">{Math.round(score*100)}%</text>
        </>}
      </g>;
    })}
  </svg>;
