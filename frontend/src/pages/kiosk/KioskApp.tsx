...
export default function KioskApp() {
  ...
  useEffect(() => {
    if (flow.currentState === "IDLE" || flow.currentState === "WELCOME" || flow.currentState === "AI_CHAT" || flow.currentState === "VOICE_CHAT") camera.stopCamera();
  ...
  const content = (() => {
    switch (flow.currentState) {
      ...
      case "FACE_UNKNOWN": return <FaceUnknownScreen
        onRegister={() => flow.transitionTo("FACE_REGISTER")}
      ...
      case "FACE_REGISTER": return <FaceRegistrationScreen videoRef={camera.videoRef} cameraStatus={camera.cameraStatus}
        cameraError={camera.error} busy={flow.isProcessing} captureFrame={camera.captureFrame} onEnroll={flow.enrollFace}
        onRetry={flow.startFaceScan} onGuest={flow.continueAsGuest}/>;
      case "AI_CHAT":
      case "VOICE_CHAT": return <KioskVoiceChatScreen flow={flow}/>;
      case "BOOK_SUGGESTION": return <KioskBookSuggestionScreen onBack={() => flow.transitionTo("VOICE_CHAT")} onSurvey={flow.openSurvey}/>;
  ...
  return <KioskChrome state={flow.currentState} onExit={flow.currentState !== "IDLE" ? () => { void flow.resetToIdle("USER_EXIT"); } : undefined}>
    {content}
    {import.meta.env.DEV && String(import.meta.env.VITE_ENABLE_DEV_CONTROLS ?? "true") === "true" && flow.currentState !== "IDLE" && <div className="kiosk-dev-panel">
      <b>DEV</b>
